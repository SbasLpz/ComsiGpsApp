...
checkChangePassword() async {
  ...
  if(passController.text.trim() == cc) {
    if(newPassController.text.trim() == confirmPassController.text.trim()){
      var result = await changePassword(cc, newPassController.text.trim());
      if(result.success!) {
        msg = "Contraseña cambiada !";
        print("--- Contraseña cambiada !: ${result.msg.toString()}");
        ...
        msg = "Error del servidor: "+result.msg.toString();
        print("--- Error del servidor: ${result.msg.toString()}");
      }
    } else {
